...
package com.wqlin.widget.flexbox;
...
import com.wqlin.android.flexbox.R;
...
/**
 * 最后一行的item总宽度与父View的宽度比例来达到多大比例时才展开 {@link #setLastLineExpand(float)}
 * <P>
 *
...
public class FlexboxLayout extends ViewGroup implements FlexContainer {
    ...
    /**
     * 最后一行的item总宽度与父View的宽度比例来达到多大比例时才展开
     */
    private float mLastLineExpand = 0f;
    ...
    /**
     * 最后一行的item总宽度与父View的宽度比例来达到多大比例时才展开
     * @param lastLineExpand [0,1]
     */
    public void setLastLineExpand(float lastLineExpand) {
        if (lastLineExpand<0 ||lastLineExpand>1) return;
        mLastLineExpand = lastLineExpand;
    }

    /**
     * line行是否展开(前提设置了FlexItem.flexGrow)
     * @param flexLines
     * @param line
     * @param mainSize
     * @return false并缩 true-->不会拉伸
     */
    @Override
    public boolean isExpand(List<FlexLine> flexLines, int line, int mainSize) {
        int count = flexLines.size();
        if (count == line + 1) {
            FlexLine flexLine=flexLines.get(line);
            if (mLastLineExpand<=0)
                return true;
            float max = mainSize * mLastLineExpand;
            if (flexLine.mMainSize<max)
                return false;
        }
        return true;
    }
